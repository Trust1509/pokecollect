...
1. **Koordinaten** — x/y/w/h bzw. jeder Quad-Punkt muss ein echter Bild-Anteil
   sein (Auflage 3). Fängt z.B. `vlm.py::_bbox`s Fallback-Skalierung: das rohe
   `[200,10,63,88]` wird dort (nur ab Werten > 1,5 wird durch 100 geteilt) zu
   `[2.0, 0.1, 0.63, 0.88]` — x=2.0 liegt weit außerhalb des Bildes. Verwerfen
   statt klammern: eine geklammerte Koordinate zeichnet aus der FALSCHEN
   Bildregion, das ist schlimmer als gar kein Hinweis.
...
# Obere Schranke (Panel-Nacharbeit #36, NEU) — ohne sie passierten ein
# 1,5-Karten-Streifen QUER (Verhältnis ~0,931) und das ganze Bild (Verhältnis
# 1,0 bei bbox=[0,0,1,1]) als "plausible Karte". Eine echte liegende
# (Querformat-)Karte liegt bei ~0,716 und bleibt klar darunter.
#
...
# Flächen-Kappe (Panel-Nacharbeit #36, NEU) — wirkt NUR bei mode multi/binder
# (Parameter `mode`, s. `sanitize_crop_hints`), im single-Modus nie: ein
# formatfüllendes Einzelfoto ist dort normal (eine Karte darf das ganze Bild
# einnehmen).
#
...
# Koordinaten-Toleranz (Panel-Nacharbeit #36, NEU): bbox/quad sind Bild-
...
def _bbox_reject_reason(bbox: list[float], img_w: float, img_h: float, mode: str) -> str | None:
    """None = plausibel; sonst der Verwerfungsgrund fürs Logging (Auflage 5)."""
    if len(bbox) != 4:
        return "koordinaten"
    x, y, w, h = bbox
...
def _quad_reject_reason(quad: list[list[float]], img_w: float, img_h: float, mode: str) -> str | None:
    ...
    if len(quad) != 4 or any(len(p) != 2 for p in quad):
        return "koordinaten"
    if not _values_in_bounds([v for p in quad for v in p]):
        return "koordinaten"
...
def sanitize_crop_hints(
    ...
    Zählt Verwerfungen getrennt nach bbox/quad und loggt EINE Zeile je
    Aufruf, wenn mindestens eine Verwerfung stattfand (Auflage 5 — "stille
    Ausfälle" sollen sichtbar sein, ohne die API zu erweitern).
    """
    ...
    quad_verworfen = 0
    for r in reads:
        if r.quad is not None and not _quad_plausible(r.quad, img_w, img_h, mode):
            r.quad = None
            quad_verworfen += 1
        if r.bbox is not None and not _bbox_plausible(r.bbox, img_w, img_h, mode):
            r.bbox = None
            bbox_verworfen += 1

    if bbox_verworfen or quad_verworfen:
        log.info(
            "Zuschnitt-Hinweise verworfen: %d bbox, %d quad (Grund je: Verhältnis/Fläche/Koordinaten)",
            bbox_verworfen, quad_verworfen,
        )
    return reads
